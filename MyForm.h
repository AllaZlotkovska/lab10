...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button1->Text = L"Зчитати з файла дані";
			...
			this->label1->Text = L"Суму чисел, кратиних 3";
			// 
			// button3
			// 
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (File::Exists(fname)) {
			...
			for (int i = 0; i < numbers->Length; i++) {
				textBox1->AppendText(numbers[i] + Environment::NewLine); // Звертання до всіх елементів масиву
			}
			reader->Close();
		}
	}
...
private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
	int sum = 0; // Змінна для обчислення суми чисел, а не добутку
	for (int i = 0; i < textBox1->Lines->Length; i++) {
		int number;
		if (int::TryParse(textBox1->Lines[i], number)) {
			if (number % 3 == 0) {
				sum = sum + number; // Заміна додавання на обчислення суми
			}
		}
	}
	label2->Text = Convert::ToString(sum); // Оновлення тексту мітки

}
	private: System::Void button3_Click(System::Object^ sender, System::EventArgs^ e) {
		String^ fileName = "output.txt";
		if (File::Exists(fileName)) {
			MessageBox::Show("Файл " + fileName + " вже існує.");
		}
		else {
			StreamWriter^ writer = File::CreateText(fileName);
			int sum = 0; // Змінна для обчислення суми
			for (int i = 0; i < textBox1->Lines->Length; i++) {
				int number;
				if (int::TryParse(textBox1->Lines[i], number)) {
					if (number % 3 == 0) {
						sum += number; // Додавання числа до суми
						writer->WriteLine(number);
					}
				}
			}
			writer->WriteLine("Сума: " + Convert::ToString(sum)); // Запис суми у файл
			writer->Close();
			MessageBox::Show("Числа, кратні 3, та їх суму було записано у файл " + fileName);
		}
	}
};
}
